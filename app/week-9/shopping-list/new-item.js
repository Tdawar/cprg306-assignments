...
export default function NewItem({ onAddItem }) {
  ...
  if (quantity >= 20) buttonStylesInc += " opacity-50 cursor-not-allowed";
  let buttonStylesDec = "mt-4 bg-blue-500 hover:bg-blue-300 text-white px-4 ml-5 py-2 rounded";
  if (quantity <= 1) buttonStylesDec += " opacity-50 cursor-not-allowed";
  ...
  return (
    <form onSubmit={handleSubmit} className="outline p-9">
      ...
      <p>
        <button className={buttonStylesDec} onClick={decrement} type="button">-</button>
        <button className={buttonStylesInc} onClick={increment} type="button">+</button>
      </p>
